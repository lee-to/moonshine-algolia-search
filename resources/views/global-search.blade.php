<div>
    ...
    <x-moonshine::modal title="{{ __('Search docs') }}">
        <div x-data="globalSearch">
            ...
            <ul>
                <template x-for="item in items" :key="item.objectID">
                    <li class="search-item">
                        <a :href="item.url" class="flex items-center justify-start space-x-4">
                            <span x-show="item.icon" x-html="item.icon"></span>
                            <div x-show="item.image" class="zoom-in h-10 w-10 overflow-hidden rounded-md">
                                <img :src="item.image" class="h-full w-full object-cover" :alt="item.title">
                            </div>

                            <div>
                                <div class="font-bold" x-text="item.title"></div>
                                <div x-show="item.section" x-text="item.section"></div>
                            </div>
                        </a>
                    </li>
                </template>
            ...
            <footer class="search-footer">
                <div class="algolia-logo flex items-center justify-end space-x-2">
                    <span>Powered by</span>
                    <a href="https://www.algolia.com/ref/docsearch/?utm_source=moonshine.cutcode.dev&amp;utm_medium=referral&amp;utm_content=powered_by&amp;utm_campaign=docsearch"
                       target="_blank"
        ...
        <x-slot:outerHtml>
            <div class="flex items-center justify-between">
                <a @click.prevent="toggleModal(); setTimeout(() => document.getElementById('globalSearch').focus(), 100)"
                   href="#"
                >
                    <x-moonshine::icon
    ...
    <script>
        function globalSearch() {
            const client = algoliasearch('{{ config('algolia.app_id') }}', '{{ config('algolia.frontend_key') }}');
            const index = client.initIndex('moonshine_search_index_{{ app()->getLocale() }}');
            ...
            return {
                ...
                search() {
                    if(this.query.length > 1) {
                        index.search(this.query, {hitsPerPage: 5})
